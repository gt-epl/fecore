...
func (s *Supervisor) Start(svcs []Service) error {
	ctx := namespaces.WithNamespace(context.Background(), FaasEdgeNamespace)
...
%s	fecore-provider`, gw)
	...
	for _, svc := range svcs {
		...
		img, err := service.PrepareImage(ctx, s.client, imgRef, svc.Name, defaultSnapshotter, faasServicesPullAlways)
		if err != nil {
			return err
		}
		images[svc.Name] = img
	...
	for _, key := range order {
		...
		if len(svc.Mounts) > 0 {
			for _, mnt := range svc.Mounts {
				...
				if len(path.Ext(mnt.Src)) == 0 &&
					!strings.HasPrefix(mnt.Src, "/var/lib/fecore/secrets/") {
		...
		task, err := newContainer.NewTask(ctx, cio.BinaryIO("/usr/local/bin/fecore", nil))
		if err != nil {
		...
		result, err := cninetwork.CreateCNINetwork(ctx, s.cni, task, labels)
		if err != nil {
...
func GetArchSuffix(getClientArch ArchGetter) (suffix string, err error) {
	...
	if clientOS != "Linux" {
		return "", fmt.Errorf("You can only use fecore with Linux")
	}
